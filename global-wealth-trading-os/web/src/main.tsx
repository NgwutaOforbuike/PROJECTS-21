...
import {AlertTriangle,BookOpen,Brain,Database,Globe2,LockKeyhole,LogOut,PlugZap,RefreshCw,Search,ShieldCheck,Target,WalletCards} from "lucide-react";
import "./styles.css";
import {buildInfo} from "./buildInfo";

type Tab="decision"|"opportunities"|"portfolio"|"risk"|"research"|"models"|"data"|"integrations";
...
function App({user,onLogout}:{user:Json;onLogout:()=>void}){
  ...
  const current=useMemo(()=>({decision,opportunities:market,portfolio,risk,research,models,data,integrations}[tab]),[tab,decision,market,portfolio,risk,research,models,data,integrations]);
    ...
    <main>
      <header>
        <div><p className="eyebrow">PERSONAL INVESTMENT COMMAND CENTRE</p><h1>{
          ...
          "Which services are genuinely connected?"
        }</h1></div>
